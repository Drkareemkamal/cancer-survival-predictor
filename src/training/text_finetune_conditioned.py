"""
Strategy 4: Cancer-Type Conditioning Token Fine-Tuning (ClinicalBERT) — Enhanced
=================================================================================
...
def train_and_extract(data_path, output_dir, max_epochs=30, batch_size=8, lr=2e-5,
    ...
    if use_wandb:
        ...
        wandb.init(project=wandb_project, name="ClinicalBERT-Conditioned-Enhanced", config={
            ...
            "gradient_clip": 1.0, "early_stop_metric": "c-index",
        })
    ...
    print(f"Dataset split: {len(train_dataset)} train / {len(val_dataset)} validation (stratified)")
    ...
    total_steps = (len(train_loader) // gradient_accumulation_steps) * max_epochs
    ...
    print(f"\nStarting training (max {max_epochs} epochs, patience={patience}, metric=C-index)...")
    for epoch in range(max_epochs):
        ...
        print(f"Epoch {epoch+1}: train_loss={avg_train:.4f}  val_loss={avg_val:.4f}  val_cindex={val_cindex:.4f}")
        if use_wandb:
            wandb.log({"train/epoch_loss": avg_train, "val/epoch_loss": avg_val,
                       "val/c_index": val_cindex, "epoch": epoch + 1})
    ...
    ax1.set_title('ClinicalBERT Conditioned Enhanced: Loss', fontsize=14)
    ...
    results_df = pd.DataFrame({
        ...
        'val_loss': val_losses, 'val_cindex': val_cindexes,
    ...
    print(f"\n{'='*60}")
    print(f"SUMMARY — Strategy 4: Cancer-Type Conditioning (ClinicalBERT) Enhanced")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    train_and_extract(
        ...
        max_epochs=30, patience=5, val_split=0.15,
